...
(() => {
  ...
  class LayeredGrid {
    ...
    static get layers() {
      return [
        {
          name: "pdiff",
          Stix: "rgba(118,   0, 181,   1)",
          Sorcery: "rgba(202, 160, 115,   1)",
          Tol: "rgba(51,   34, 136,   1)",
          selectable: true,
        },
        {
          name: "ndiff",
          Stix: "rgba(50,  205,  50,   1)",
          Sorcery: "rgba( 66, 213,  66,   1)",
          Tol: "rgba(17,  119,  51,   1)",
          selectable: true,
        },
        {
          name: "poly",
          Stix: "rgba(255,   0,   0, 0.6)",
          Sorcery: "rgba(220,  95,  95, 0.6)",
          Tol: "rgba(136,  34,  85, 0.6)",
          selectable: true,
        },
        {
          name: "metal1",
          Stix: "rgba(0,   255, 255, 0.6)",
          Sorcery: "rgba(125, 166, 250, 0.6)",
          Tol: "rgba(136, 204, 238, 0.6)",
          selectable: true,
        },
        {
          name: "metal2",
          Stix: "rgba(255,   0, 204, 0.6)",
          Sorcery: "rgba(190, 153, 222, 0.6)",
          Tol: "rgba(221, 204, 119, 0.6)",
          selectable: true,
        },
        {
          name: "contact",
          Stix: "rgba(204, 204, 204, 0.5)",
          Sorcery: "rgba(204, 204, 204, 0.5)",
          Tol: "rgba(204, 102, 119, 0.5)",
          selectable: true,
        },
        {
          name: "delete",
          Stix: "rgba(208, 160,  32, 0.5)",
          Sorcery: "rgba(230, 230,   0, 0.5)",
          Tol: "rgba(170,  68, 153, 0.5)",
          selectable: false,
        },
      ];
    }
    ...
    static get DELETE() {
      return 6;
    } // always make this the last layer
    ...
    set(x, y, layer) {
      ...
      // Only allow one of NDIFF or PDIFF to be set if(layer === LayeredGrid.PDIFF) {
    ...
    isTerminal(x, y, layer) {
      ...
      isTerminal =
        layer === LayeredGrid.CONTACT &&
        this.diagram.inputs.some(function (input) {
      ...
      isTerminal =
        isTerminal ||
        (layer === LayeredGrid.CONTACT &&
          this.diagram.outputs.some(function (output) {
            return output.x === x && output.y === y;
          }));

      isTerminal =
        isTerminal ||
        (this.diagram.vddCell.x === x && this.diagram.vddCell.y === y);
      isTerminal =
        isTerminal ||
        (this.diagram.gndCell.x === x && this.diagram.gndCell.y === y);
    ...
    clear(x, y, layer) {
      ...
      if (
        outOfBounds ||
        (layer === LayeredGrid.CONTACT && this.isTerminal(x, y, layer))
      ) {
        return;
      }
    ...
    convertToCoordinates(index) {
      ...
      let y = Math.floor(
        (index - layer * this.width * this.height) / this.width,
      );
        ...
        for (let y = bounds.top; y <= bounds.bottom; y++) {
          for (let x = bounds.left; x <= bounds.right; x++) {
            ...
            if (
              cell ||
              includeEmpty ||
              (bounds.cursor && bounds.cursor.x === x && bounds.cursor.y === y)
            ) {
              func(x, y, layer);
            }
          }
    ...
    resize(width, height) {
      ...
      for (
        let layer = oldBounds.lowLayer;
        layer <= oldBounds.highLayer;
        layer++
      ) {
        for (let y = oldBounds.top; y <= oldBounds.bottom; y++) {
          for (let x = oldBounds.left; x <= oldBounds.right; x++) {
            this.grid[this.convertFromCoordinates(x, y, layer)] =
              oldGrid[x + y * oldWidth + layer * oldWidth * oldHeight];
          }
        }
      }

      // Move inputs, outputs, and VDD/GND if they are outside the new grid
      this.diagram.inputs.forEach(
        function (input) {
          this.moveWithinBounds(input, newBounds);
        }.bind(this),
      );
      this.diagram.outputs.forEach(
        function (output) {
      ...
      for (let index = 0; index < this.grid.length; index++) {
        ...
        oldCell =
          oldGrid[
            coords.x +
              coords.y * this.width +
              coords.layer * this.width * this.height
          ];
        ...
        if (byRow) {
          ...
          isInShiftRange = Boolean(
            this.coordsAreInBounds(0, coords.y - yOffset - startY),
          );
          ...
          isInShiftRange = Boolean(
            this.coordsAreInBounds(coords.x - xOffset - startX, 0),
          );
        ...
        // Offsets the start point depending on whether this is an insertion or deletion.
          ...
          if (
            offsetCell &&
            this.coordsAreInBounds(coords.x - xOffset, coords.y - yOffset)
          ) {
            this.set(coords.x, coords.y, coords.layer);
          }
        }
        ...
        // Don't extend CONTACT layer.
          ...
          cellExtendable &&
          extendCell &&
          coords.layer !== LayeredGrid.CONTACT
        ) {
          this.set(coords.x, coords.y, coords.layer);
        }
      }
  ...
  class Net {
    ...
    addCell(cell, contactSet) {
      ...
      if (!contactSet) {
        ...
        this.hasDiff =
          this.hasDiff ||
          cell.layer === LayeredGrid.NDIFF ||
          cell.layer === LayeredGrid.PDIFF;
      }
    }
  ...
  class Node {
    ...
    isConnected(otherNode) {
      for (let ii = 0; ii < this.edges.length; ii++) {
        if (
          this.edges[ii].getNode1() === otherNode ||
          this.edges[ii].getNode2() === otherNode
        ) {
          return true;
        }
      }
  ...
  class DiagramController {
    ...
    removeTerminal(isOutput) {
      ...
      if (removedTerm !== undefined) {
        this.diagram.layeredGrid.clear(
          removedTerm.x,
          removedTerm.y,
          LayeredGrid.CONTACT,
        );
    ...
    addTerminal(isOutput) {
      ...
      if (
        this.diagram.inputs.length + this.diagram.outputs.length >=
        this.diagram.maxTerminals
      ) {
        return;
      }
    ...
    saveCurrentState() {
      ...
      localStorage.setItem(
        "layeredGrid" + this.saveState,
        JSON.stringify(this.diagram.layeredGrid.grid),
      );
    ...
    undo() {
      ...
      if (this.saveState > this.firstSaveState) {
        this.saveState--;
        this.diagram.layeredGrid.grid = JSON.parse(
          localStorage.getItem("layeredGrid" + this.saveState),
        );
      }
    }
    ...
    pixelIsInBounds() {
      let x = Math.floor(
        (this.currentX -
          this.view.canvas.getBoundingClientRect().left -
          this.view.cellWidth) /
          this.view.cellWidth,
      );
      let y = Math.floor(
        (this.currentY -
          this.view.canvas.getBoundingClientRect().top -
          this.view.cellHeight) /
          this.view.cellHeight,
      );

      return (
        x >= 0 &&
        x < this.diagram.layeredGrid.width &&
        y >= 0 &&
        y < this.diagram.layeredGrid.height
      );
    }
    ...
    getCellAtCursor() {
      ...
      if (this.pixelIsInBounds(this.currentX, this.currentY)) {
        ...
        this.currentCell = {
          ...
          contact: this.diagram.layeredGrid.get(x, y, LayeredGrid.CONTACT)
            .isSet,
    ...
    draw(bounds) {
      if (
        Math.abs(bounds.endX - this.startX) >
        Math.abs(bounds.endY - this.startY)
      ) {
        bounds.lowLayer = bounds.highLayer = this.cursorIndex;
        bounds.bottom = bounds.top = this.startY;
        this.diagram.layeredGrid.map(
          bounds,
          function (x, y, layer) {
    ...
    isPrimaryInput(event) {
      ...
      let isLeftButton =
        ...
        event.button === 0;
      isLeftButton =
        isLeftButton || (event.type === "mousemove" && event.buttons === 1);
    ...
    placeTerminal(event, terminal, useGridCoords) {
      ...
      const hasOwn = Object.hasOwn
        ? Object.hasOwn(cell, "x")
        : cell.hasOwnProperty("x"); // compatibility
    ...
    changeLayer(layerIndex) {
      if (layerIndex === undefined) {
        ...
        while (
          tempIndex >= LayeredGrid.layers.length ||
          !LayeredGrid.layers[tempIndex].selectable
        ) {
          tempIndex =
            tempIndex >= LayeredGrid.layers.length - 1 ? 0 : tempIndex + 1;
        }
  ...
  class DiagramView {
    ...
    constructor(diagram, mainCanvas) {
      ...
      this.cellHeight =
        this.canvasHeight / (this.diagram.layeredGrid.height + 2);
    ...
    drawGrid() {
      ...
      for (
        ...
        ii <=
        Math.max(
          this.diagram.layeredGrid.width,
          this.diagram.layeredGrid.height,
        );
        ii++
      ) {
        if (ii <= this.diagram.layeredGrid.width) {
          this.ctx.beginPath();
          this.ctx.moveTo(ii * this.cellWidth, this.cellHeight);
          this.ctx.lineTo(
            ii * this.cellWidth,
            this.canvasHeight - this.cellHeight,
          );
          this.ctx.stroke();
        }
        if (ii <= this.diagram.layeredGrid.height) {
          this.ctx.beginPath();
          this.ctx.moveTo(this.cellWidth, ii * this.cellHeight);
          this.ctx.lineTo(
            this.canvasWidth - this.cellWidth,
            ii * this.cellHeight,
          );
          this.ctx.stroke();
        }
    ...
    drawBorder() {
      ...
      if (this.diagram.controller.eraseMode) {
        ...
        this.ctx.strokeStyle = this.getColor(
          this.diagram.controller.cursorIndex,
        );
      }
      ...
      this.ctx.strokeRect(
        ...
        this.cellWidth / 2,
        this.canvasWidth - this.cellWidth,
        this.canvasHeight - this.cellWidth,
      );
      ...
      this.ctx.strokeRect(
        1 + this.cellWidth - this.ctx.lineWidth / 2,
      ...
      if (this.diagram.controller.eraseMode) {
        ...
        this.ctx.fillText(
          LayeredGrid.layers[this.diagram.controller.cursorIndex].name,
          this.canvasWidth / 2,
          (this.cellHeight * 3) / 4,
        );
      }
    ...
    decorateContact(x, y) {
      ...
      this.ctx.beginPath();
      this.ctx.moveTo(
        x * this.cellWidth + this.cellWidth + 1,
        y * this.cellHeight - 1,
      );
      this.ctx.lineTo(
        x * this.cellWidth,
        y * this.cellHeight + this.cellHeight + 1,
      );
      this.ctx.stroke();
      this.ctx.beginPath();
      this.ctx.moveTo(
    ...
    drawLabels() {
      ...
      this.diagram.getTerminals().forEach(
        ((terminal, index) => {
          this.ctx.fillText(
            this.diagram.getTerminalName(index),
    ...
    drawCell(ii, jj, layer) {
      let currentCell, hoverCell, isCurrentRow, isCurrentCol, isCurrentCell;
      ...
      if (
        this.diagram.controller.dragging ||
        this.diagram.controller.placeTermMode
      ) {
    ...
    setCellHighlight(ctx, x, y) {
      if (
        this.highlightNets &&
        this.netHighlightGrid[x] &&
        this.netHighlightGrid[x][y]
      ) {
        let baseColor = this.getColor(LayeredGrid.DELETE, false).slice(0, -4);
        ctx.fillStyle =
          baseColor +
          Math.round(
            Math.sin(((Math.floor(Date.now() / 100) % 10) * Math.PI) / 10) * 10,
          ) /
            10 +
          ")";
        ctx.fillRect(
          (x + 1) * this.cellWidth,
          (y + 1) * this.cellHeight - 1,
          this.cellWidth + 1,
          this.cellHeight + 2,
        );
      }
    ...
    setPathHighlight(path) {
      ...
      for (let ii = 0; ii < path.length; ii++) {
        ...
        for (let jj = 0; jj < this.diagram.netlist.length; jj++) {
          if (
            this.diagram.netlist[jj].containsNode(this.diagram.graph.nodes[ii])
          ) {
    ...
    refreshCanvas(printOnly = false) {
      ...
      let currentCell = printOnly
        ? {}
        : this.diagram.controller.getCellAtCursor();
      if (currentCell.contact) {
        document.getElementById("CONTACT").style.backgroundColor =
          this.getColor(LayeredGrid.CONTACT, true);
      } else {
        document.getElementById("CONTACT").style.backgroundColor =
          "transparent";
      }
      ...
      if (currentCell.metal2) {
        document.getElementById("METAL2").style.backgroundColor = this.getColor(
          LayeredGrid.METAL2,
          true,
        );
      } else {
      ...
      if (currentCell.metal1) {
        document.getElementById("METAL1").style.backgroundColor = this.getColor(
          LayeredGrid.METAL1,
          true,
        );
      } else {
      ...
      if (currentCell.poly) {
        document.getElementById("POLY").style.backgroundColor = this.getColor(
          LayeredGrid.POLY,
          true,
        );
      } else {
      ...
      if (currentCell.pdiff) {
        document.getElementById("DIFF").style.backgroundColor = this.getColor(
          LayeredGrid.PDIFF,
          true,
        );
      ...
      document.querySelector("#num-rows").innerHTML =
        this.diagram.layeredGrid.height;
      document.querySelector("#num-cols").innerHTML =
        this.diagram.layeredGrid.width;
  ...
  class Diagram {
    constructor(mainCanvas) {
      ...
      this.view = new DiagramView(this, mainCanvas);
    ...
    encode() {
      ...
      // Terminal locations
      [this.vddCell, this.gndCell]
        .concat(this.inputs.concat(this.outputs))
    ...
    decode(stringBase64) {
      const decompress = atob(
        window.LZUTF8.decompress(stringBase64, { inputEncoding: "Base64" }),
      );
      const setGrid = [];
      let byte,
        bitShift,
      ...
      [this.inputs, this.outputs].forEach(
        function (arr) {
          for (let ii = 0; ii < arr.length; ii++) {
            arr[ii] = {
              x: setGrid.splice(0, 1)[0],
              y: setGrid.splice(0, 1)[0],
            };
      ...
      for (
        let lyr = 0;
        lyr < this.layeredGrid.layers - 1 && lyr < setDepth;
        lyr++
      ) {
        for (let col = 0; col < this.layeredGrid.width; col++) {
          for (let row = 0; row < this.layeredGrid.height; row++) {
    ...
    initCells() {
      ...
      this.layeredGrid = new LayeredGrid(
        this,
        startWidth,
        startHeight,
        LayeredGrid.layers.length,
      );
    }
    ...
    getTerminalName(index) {
      if (index === 0) {
        ...
        return String.fromCharCode(
          90 - this.outputs.length + index - this.inputs.length - 2,
        );
      } else {
        return "?";
      }
    ...
    getMapping(node1, node2) {
      return this.nodeNodeMap[this.graph.getIndexByNode(node1)][
        this.graph.getIndexByNode(node2)
      ];
    }
    ...
    mapNodes(node1, node2, setPath) {
      ...
      this.nodeNodeMap[this.graph.getIndexByNode(node1)][
        this.graph.getIndexByNode(node2)
      ] = setPath;
      this.nodeNodeMap[this.graph.getIndexByNode(node2)][
        this.graph.getIndexByNode(node1)
      ] = setPath;
      ...
      if (setPath === this.COMPUTING_PATH) {
        return;
      }
    ...
    updateVirtualPath(compareNodeMapping, nodeToMap, remapNode, setPath) {
      if (compareNodeMapping === this.DIRECT_PATH) {
        if (setPath === this.DIRECT_PATH) {
          this.remap(
            nodeToMap,
            this.graph.getIndexByNode(remapNode),
            this.DIRECT_PATH,
          );
        ...
        compareNodeMapping === this.NO_PATH &&
        setPath === this.DIRECT_PATH
      ) {
        this.remap(
          nodeToMap,
          this.graph.getIndexByNode(remapNode),
    ...
    updateNoPath(compareNode, nodeToMap, remapNode, setPath) {
      let compareNodeMapping =
        this.nodeNodeMap[nodeToMap][this.graph.getIndexByNode(compareNode)];
      let mapFromRail = this.isRailNode(compareNode);

      if (
        !mapFromRail &&
        setPath === this.DIRECT_PATH &&
        compareNodeMapping.direct === false
      ) {
        this.remap(
          nodeToMap,
          this.graph.getIndexByNode(remapNode),
    ...
    syncEdges(nodeToMap, compareNode, remapNode, setPath) {
      ...
      let compareNodeMapping =
        this.nodeNodeMap[nodeToMap][this.graph.getIndexByNode(compareNode)];
      let remapNodeMapping =
        this.nodeNodeMap[nodeToMap][this.graph.getIndexByNode(remapNode)];

      // Case 0: Insufficient information to remap nodes.
      if (
        compareNodeMapping.hasPath === undefined ||
        setPath.hasPath === undefined
      ) {
        return;
      }
    ...
    deactivateGate(node) {
      this.graph.nodes.forEach(
        function (otherNode) {
          ...
          this.nodeNodeMap[this.graph.getIndexByNode(node)][
            this.graph.getIndexByNode(otherNode)
          ] = this.NO_PATH;
          this.nodeNodeMap[this.graph.getIndexByNode(otherNode)][
            this.graph.getIndexByNode(node)
          ] = this.NO_PATH;
        }.bind(this),
      );
    }
    ...
    attemptGateConflictResolution(node, targetNode, inputVals) {
      let targetNodeReachable,
        nodeTerm1,
        nodeTerm2,
        od,
        gndPathExistsActivated,
        vddPathExistsActivated;
      ...
      targetNodeReachable = this.recurseThroughEdges(
        node,
        targetNode,
        inputVals,
      ).hasPath;
      ...
      if (
        targetNodeReachable &&
        Math.min(node.cell.term1.nodes.size, node.cell.term2.nodes.size) > 1
      ) {
        gndPathExistsActivated = this.recurseThroughEdges(
          nodeTerm1,
          this.gndNode,
          inputVals,
        );
        vddPathExistsActivated = this.recurseThroughEdges(
          nodeTerm1,
          this.vddNode,
          inputVals,
        );
        ...
        if (
          !gndPathExistsActivated.hasPath ||
          !vddPathExistsActivated.haspPath
        ) {
    ...
    recurseThroughEdges(node, targetNode, inputVals) {
      ...
      node.edges.some(
        function (edge) {
          ...
          if (
            !mapping.direct &&
            (otherNode === this.vddNode || otherNode === this.gndNode)
          ) {
            return false;
          }
          ...
          if (mapping === this.UNCHECKED) {
            mapping = this.computeOutputRecursive(
              otherNode,
              targetNode,
              inputVals,
            );
    ...
    computeOutputRecursive(node, targetNode, inputVals) {
      ...
      if (
        this.inputNodes.indexOf(node) >= 0 &&
        this.evaluateInput(node, inputVals) === targetNode
      ) {
    ...
    evalInputDrivenGate(node, inputVals, gateNet) {
      ...
      this.overdrivenPath =
        ...
        (gateNet.containsNode(this.vddNode) &&
          gateNet.containsNode(this.gndNode));

      evalInput =
        (evalInput || gateNet.containsNode(this.vddNode)) &&
        !gateNet.containsNode(this.gndNode);
    ...
    gateIsActive(node, inputVals) {
      ...
      for (
        let connectedNode = connectedNodeIterator.next();
        !connectedNode.done;
        connectedNode = connectedNodeIterator.next()
      ) {
        ...
        relevantPathExists = this.computeOutputRecursive(
          connectedNode,
          relevantNode,
          inputVals,
        );
        oppositePathExists = this.computeOutputRecursive(
          connectedNode,
          oppositeNode,
          inputVals,
        );
      ...
      if (!!this.analyses[inputVals] && !!this.analyses[inputVals].length) {
        ...
        this.graph.nodes.forEach(
          function (_, ii) {
            ...
            this.graph.nodes.forEach(
              function (_, jj) {
                this.nodeNodeMap[ii][jj] =
                  ii === jj ? this.DIRECT_PATH : this.UNCHECKED;
              }.bind(this),
            );
          }.bind(this),
        );
    ...
    forEachTransistor(funct) {
      ...
      transistorLists.forEach(
        function (transistorList) {
          let iterator = transistorList.values();

          for (
            let transistor = iterator.next();
            !transistor.done;
            transistor = iterator.next()
          ) {
    ...
    resetNetlist() {
      ...
      this.inputNets.forEach(
        function (net) {
          this.netlist.push(net);
        }.bind(this),
      );
      this.outputNets.forEach(
        function (net) {
          this.netlist.push(net);
        }.bind(this),
      );
    ...
    setNets() {
      ...
      this.vddNode = this.graph.addNode(
        this.layeredGrid.get(
          this.vddCell.x,
          this.vddCell.y,
          LayeredGrid.CONTACT,
        ),
      ...
      this.gndNode = this.graph.addNode(
        this.layeredGrid.get(
          this.gndCell.x,
          this.gndCell.y,
          LayeredGrid.CONTACT,
      ...
      this.inputs.forEach(
        function (input, index) {
          this.setRecursively(
            this.layeredGrid.get(input.x, input.y, LayeredGrid.CONTACT),
            this.inputNets[index],
          );
        }.bind(this),
      );
      ...
      this.outputs.forEach(
        function (output, index) {
          this.setRecursively(
            this.layeredGrid.get(output.x, output.y, LayeredGrid.CONTACT),
            this.outputNets[index],
          );
        }.bind(this),
      );
      ...
      this.inputs.forEach(
        function (input, index) {
          this.inputNodes[index] = this.graph.addNode(
            this.layeredGrid.get(input.x, input.y, LayeredGrid.CONTACT),
            true,
          );
      ...
      this.outputs.forEach(
        function (output, index) {
          this.outputNodes[index] = this.graph.addNode(
            this.layeredGrid.get(output.x, output.y, LayeredGrid.CONTACT),
            true,
          );
    ...
    processTransistors() {
      ...
      this.forEachTransistor(
        function (transistor, _, term) {
      ...
      this.forEachTransistor(
        function (_, transistor, termA) {
          ...
          this.forEachTransistor(function (_, transistor2, termB) {
            ...
            if (termB === "gate" || transistor === transistor2) {
              return;
            }
    ...
    checkIfTransistor(cell) {
      ...
      let handleCell = function (layer, transistorArray) {
        if (!transistorArray.has(cell) && cell.layer === layer && cell.isSet) {
          if (
            this.layeredGrid.get(cell.x, cell.y, LayeredGrid.POLY).isSet &&
            !this.layeredGrid.get(cell.x, cell.y, LayeredGrid.CONTACT).isSet
          ) {
      ...
      return (
        handleCell(LayeredGrid.PDIFF, this.pmos) ||
        handleCell(LayeredGrid.NDIFF, this.nmos)
      );
    }
    ...
    setAdjacent(deltaX, deltaY, net, cell) {
      ...
      if (
        net.containsCell(this.layeredGrid.get(cell.x, cell.y, cell.layer)) &&
        this.layeredGrid.get(cell.x + deltaX, cell.y + deltaY, cell.layer).isSet
      ) {
        if (
          net.containsCell(
            this.layeredGrid.get(cell.x + deltaX, cell.y + deltaY, cell.layer),
          ) === false
        ) {
          this.setRecursively(
            this.layeredGrid.get(cell.x + deltaX, cell.y + deltaY, cell.layer),
      ...
      if (this.layeredGrid.get(cell.x, cell.y, LayeredGrid.CONTACT).isSet) {
        LayeredGrid.layers.forEach(
          function (_, layer) {
            ...
            if (
              net.containsCell(this.layeredGrid.get(cell.x, cell.y, layer)) ===
              false
            ) {
              net.addCell(this.layeredGrid.get(cell.x, cell.y, layer), true);
              this.setRecursively(
                this.layeredGrid.get(cell.x, cell.y, layer),
                net,
              );
            }
          }.bind(this),
    ...
    setRecursively(cell, net) {
      ...
      if (this.checkIfTransistor(cell)) {
        gateNet = this.getNet(
          this.layeredGrid.get(cell.x, cell.y, LayeredGrid.POLY),
        );
      ...
      // Check the cells above and below.
      if (cell.y > 0) {
        this.setAdjacent(0, -1, net, cell);
      }
      if (cell.y < this.layeredGrid.height - 1) {
        this.setAdjacent(0, 1, net, cell);
      }

      // Check the cells to the left and right.
      if (cell.x > 0) {
        this.setAdjacent(-1, 0, net, cell);
      }
      if (cell.x < this.layeredGrid.width - 1) {
        this.setAdjacent(1, 0, net, cell);
      }
    }
  }
  ...
  class UserInterface {
    ...
    keydownHandler(event) {
      ...
      if (event.shiftKey && this.shiftCommands[event.keyCode]) {
        ...
        event.preventDefault();
        this.diagramController.placeTerminal(
          event,
          this.diagram.inputs[event.keyCode - 65],
        );
        this.update = true;
        ...
        this.diagramController.placeTerminal(
          event,
          this.diagram.outputs[
            this.diagram.outputs.length - 90 + event.keyCode
          ],
        );
        this.update = true;
      }
    ...
    keyupHandler(event) {
      ...
      if (
        !event.shiftKey &&
        !event.ctrlKey &&
        this.noModifierCommands[event.keyCode]
      ) {
      ...
      this.deleteColCommand = {
        ...
        action: function (e) {
          if (e.type.includes("up")) {
            ...
            const hasOwn = Object.hasOwn
              ? Object.hasOwn(coords, "x")
              : coords.hasOwnProperty("x"); // compatibility
            coords = hasOwn ? coords : { x: this.diagramGrid.width };
            if (hasOwn) {
              this.diagramGrid.insertRemoveRowColAt(coords.x, false, false);
            } else {
              this.diagramGrid.resize(
                this.diagramGrid.width - 1,
                this.diagramGrid.height,
              );
            }
          }
        }.bind(this),
      ...
      this.deleteRowCommand = {
        ...
        action: function (e) {
          if (e.type.includes("up")) {
            let coords = this.diagramController.getCellAtCursor();
            const hasOwn = Object.hasOwn
              ? Object.hasOwn(coords, "y")
              : coords.hasOwnProperty("y"); // compatibility
            if (hasOwn) {
              this.diagramGrid.insertRemoveRowColAt(coords.y, false, true);
            } else {
              this.diagramGrid.resize(
                this.diagramGrid.width,
                this.diagramGrid.height - 1,
              );
            }
          }
        }.bind(this),
    ...
    initCosmeticCommands() {
      ...
      this.printCommand = {
        ...
        action: function (e) {
          ...
          this.refreshTruthTable();
          document.querySelector("#print-table").innerHTML =
            document.querySelector("#truth-table").innerHTML;
          window.print();
          this.setDarkMode(tempDarkMode);
        }.bind(this),
      };
      ...
      this.allCommands.push(this.printCommand);
    }

    changeTheme() {
      this.diagramView.theme =
        this.diagramView.theme < DiagramView.themes.length - 1
          ? this.diagramView.theme + 1
          : 0;
      document.getElementById("palette-setting").innerHTML =
        DiagramView.themes[this.diagramView.theme];
    ...
    toggleTransparency() {
      ...
      document.getElementById("transparency-setting").innerHTML = this
        .diagramView.useFlatColors
        ? "OFF"
        : "ON";
      this.update = true;
    }
    ...
    cellClickHandler(event) {
      ...
      if (controller.placeTermMode) {
        ...
        if (
          !(
            controller.clearIfPainted(coords.x, coords.y) ||
    ...
    mouseupHandler(event) {
      ...
      if (this.diagramController.isPrimaryInput(event) || event.button === 2) {
        if (
          this.diagramController.isPrimaryInput(event) &&
          this.diagramController.placeTermMode
        ) {
          this.diagramController.placeTerminal(
            event,
            this.diagramController.selectedTerminal,
          );
        }
    ...
    drag(event) {
      ...
      if (!controller.dragging) {
        ...
        if (
          controller.currentX === controller.startX &&
          controller.currentY === controller.startY
        ) {
          return;
        }
      ...
      endX = Math.floor(
        (controller.currentX -
          view.canvas.getBoundingClientRect().left -
          view.cellWidth) /
          view.cellWidth,
      );
      endY = Math.floor(
        (controller.currentY -
          view.canvas.getBoundingClientRect().top -
          view.cellHeight) /
          view.cellHeight,
      );
      ...
      if (
        this.isEraseEvent(event) ||
        controller.cursorIndex !== LayeredGrid.CONTACT
      ) {
        bounds = {
    ...
    endDrag(event) {
      ...
      let endX = Math.floor(
        (coords.x - canvas.getBoundingClientRect().left - view.cellWidth) /
          view.cellWidth,
      );
      let endY = Math.floor(
        (coords.y - canvas.getBoundingClientRect().top - view.cellHeight) /
          view.cellHeight,
      );
      let bounds = {
    ...
    mousedownHandler(event) {
      ...
      if (controller.isPrimaryInput(event) || event.button === 2) {
        ...
        if (controller.pixelIsInBounds()) {
          event.preventDefault();
          controller.startX = Math.floor(
            (coords.x - canvas.getBoundingClientRect().left - view.cellWidth) /
              view.cellWidth,
          );
          controller.startY = Math.floor(
            (coords.y - canvas.getBoundingClientRect().top - view.cellHeight) /
              view.cellHeight,
          );
        } else {
    ...
    addListeners() {
      ...
      window.addEventListener(
        ...
        function (e) {
          if (
            document.getElementById("main-menu").classList.contains("closed")
          ) {
      ...
      window.addEventListener("contextmenu", function (e) {
        if (
          e.button === 2 &&
          this.document.getElementById("main-menu").classList.contains("closed")
        ) {
    ...
    setUpControls() {
      let resizeGridByOne = function (byRow, positive) {
        return function () {
          ...
          let newWidth = byRow
            ? this.diagramGrid.width
            : this.diagramGrid.width + offset;
          let newHeight = byRow
            ? this.diagramGrid.height + offset
            : this.diagramGrid.height;
      ...
      this.menus.forEach(
        function (menuName) {
          document.getElementById("open-" + menuName + "-btn").onclick =
            this.getOpenMenuFunction(menuName);
          document.getElementById("close-" + menuName + "-btn").onclick =
            this.getCloseMenuFunction(menuName);
        }.bind(this),
      );
      ...
      document.getElementById("open-qrcode-menu-btn").onclick = function () {
        let url =
          window.location.href.split("?")[0] + "?d=" + this.diagram.encode();
      ...
      document.getElementById("remove-row").onclick = resizeGridByOne(
        true,
        false,
      );
      document.getElementById("add-column").onclick = resizeGridByOne(
        false,
        true,
      );
      document.getElementById("remove-column").onclick = resizeGridByOne(
        false,
        false,
      );

      document.getElementById("shift-left").onclick = simpleGridShift(
        false,
        false,
      );
      document.getElementById("shift-right").onclick = simpleGridShift(
        false,
        true,
      );
      document.getElementById("shift-up").onclick = simpleGridShift(
        true,
        false,
      );
      document.getElementById("shift-down").onclick = simpleGridShift(
        true,
        true,
      );
      ...
      document
        .querySelectorAll("#paint-mode-btn,#paint-mode-btn-label")
        .forEach(
          (item) =>
    ...
    getOpenMenuFunction(menuName) {
      return function () {
        const classList = document.querySelector(`#${menuName}`).classList;
        classList.remove("landscape:translate-x-full");
        classList.remove("portrait:translate-y-full");
    ...
    getCloseMenuFunction(menuName) {
      return function () {
        const classList = document.querySelector(`#${menuName}`).classList;
        classList.add("landscape:translate-x-full");
        classList.add("portrait:translate-y-full");
    ...
    buildTruthTable() {
      ...
      for (let ii = 0; ii < Math.pow(2, this.diagram.inputs.length); ii++) {
        ...
        for (let jj = 0; jj < this.diagram.outputs.length; jj++) {
          tableOutputRow[jj] = this.diagram.computeOutput(
            ii,
            this.diagram.outputNodes[jj],
          );
    ...
    refreshTruthTable(suppressSetNets) {
      ...
      col.span = this.diagram.inputs.length;
      col.classList.add("input-vals");
      colgroup.appendChild(col);
      col = document.createElement("COL");
      col.span = this.diagram.outputs.length;
      col.classList.add("output-vals");
      colgroup.appendChild(col);
        ...
        function (row, rowIndex) {
          ...
          row.forEach(
            function (cell, colIndex) {
              ...
              // Make output cells clickable to highlight paths.
              if (colIndex >= this.diagram.inputs.length) {
                tCell.onclick = function (rowIndex, colIndex) {
                  return function () {
                    ...
                    outputNodeIndex = this.diagram.graph.getIndexByNode(
                      this.diagram.outputNodes[outputNum],
                    );
    ...
    setUpLayerSelector() {
      ...
      document.querySelectorAll(".palette i").forEach(
        function (element, index) {
          ...
          element.style.color = this.diagramView.getColor(colorIndex, true);
        }.bind(this),
      );
    }
    ...
    isEraseEvent(event) {
      return (
        this.diagramController.eraseMode ||
        event.button === 2 ||
        event.buttons === 2
      );
    }
